...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDatabase
    {
        ...
        public object BatchInsert<T>(List<T> pocos)
        {
            var pd = PocoData.ForType(typeof(T), _defaultMapper);
            return BatchInsert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, pocos);
        }
        public object BatchInsert<T>(string tableName, string primaryKeyName, bool autoIncrement, List<T> pocos)
        {
            try
            {
                OpenSharedConnection();
                            ...
                            foreach (var poco in pocos)
                            {
                                ...
                                foreach (var i in pd.Columns)
                                {
                                        ...
                                        string.Compare(i.Key, primaryKeyName, true) == 0)
                                    {
                                        ...
                                        //{
                                        //    if (index == 0) names.Add(i.Key);
                                        //    valueCol.Add(string.Format("{0}.nextval", pd.TableInfo.SequenceName));
                                        //}
                                        continue;
                                    }

                                    if (theFirstOne) names.Add(Provider.EscapeSqlIdentifier(i.Key));
                                    valueCol.Add(string.Format("{0}{1}", _paramPrefix, index++));
                                    AddParam(cmd, i.Value.GetValue(poco), i.Value.PropertyInfo);
                                }
                                theFirstOne = false;
                                values.Add(string.Format("({0})", string.Join(",", valueCol.ToArray())));
